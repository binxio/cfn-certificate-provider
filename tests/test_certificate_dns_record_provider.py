...
def test_retrieval_of_dns_record(certificate):
    ...
    request = Request('Create', certificate['CertificateArn'], certificate['SubjectAlternativeNames'][1])
    response = handler(request, {})
    assert response['Status'] == 'SUCCESS', response['Reason']
    assert 'Name' in response['Data']
    assert 'Type' in response['Data']
...
class Request(dict):
    ...
    def __init__(self, request_type, certificate_arn, domain_name=None, physical_resource_id=None):
